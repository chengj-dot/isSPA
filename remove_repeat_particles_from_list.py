...
def main():
	(inlst1, center, euler_thres, output) =  parse_command_line()
	g = open(inlst1, "r")
	...
	num=0
        ...
        b=[0]*len(inlst_line1)
        oo2=open(output,"w")
       # oo2=open("repeat_particles.txt","w")	
        k=3
	for i in range(0,len(inlst_line1)):
               # num = int(inlst_line1[i].split('\t')[0])
               x1=float(inlst_line1[i].split('\t')[6].split('=')[1].split(',')[0])
               y1=float(inlst_line1[i].split('\t')[6].split('=')[1].split(',')[1])
               alt=float(inlst_line1[i].split('\t')[5].split('=')[1].split(',')[0])*PI/180
               az=float(inlst_line1[i].split('\t')[5].split('=')[1].split(',')[1])*PI/180
               phi=float(inlst_line1[i].split('\t')[5].split('=')[1].split(',')[2])*PI/180
               score1=float(inlst_line1[i].split('\t')[7].split('=')[1])
               if(a[i]==1):
                    continue
               for j in range(i+1,len(inlst_line1)):
                    if((i+1)==len(inlst_line1) or a[j]==1):
                           continue
                    x2=float(inlst_line1[j].split('\t')[6].split('=')[1].split(',')[0])
                    y2=float(inlst_line1[j].split('\t')[6].split('=')[1].split(',')[1])
                    dist=abs(math.sqrt((x2-x1)*(x2-x1)+(y2-y1)*(y2-y1)))
                    
                    score2=float(inlst_line1[j].split('\t')[7].split('=')[1])
                    if(dist<center):
                       alt_o=float(inlst_line1[j].split('\t')[5].split('=')[1].split(',')[0])*PI/180
                       az_o=float(inlst_line1[j].split('\t')[5].split('=')[1].split(',')[1])*PI/180
                       phi_o=float(inlst_line1[j].split('\t')[5].split('=')[1].split(',')[2])*PI/180
                    # origin eulerrmrix
         ...
         # to quaternion
                       ...
                       if(euler_dist<euler_thres):
                             ...
                             if(score1<=score2 and a[i]==0):
                                  a[i]=1  
        for k in range(0,len(inlst_line1)):
               if(a[k]==0):
                    oo2.write(inlst_line1[k])  
...
def parse_command_line():
	usage="%prog <input1> <center thres> <euler thres> <output>"
	parser = OptionParser(usage=usage, version="%1")
	
	if len(sys.argv)<5: 
		print "<input1> <center> <euler_thres> <output>"
		sys.exit(-1)
	...
	inlst1 = args[0]
        center = float(args[1])
        euler_thres = float(args[2])
        output = args[3]
	return (inlst1, center, euler_thres, output)
