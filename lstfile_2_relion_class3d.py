...
def main():
	(lstfile, filename,ny,apix,output, meta,debug) =  parse_command_line()
	g = open(lstfile, "r")
...
def parse_command_line():
	usage="%prog <lstfile> <spi or mrcs filename> <ny> <apix> <output> < metadata line> <1,odd 2,even 0,none>"
	parser = OptionParser(usage=usage, version="%1")
	
	if len(sys.argv)<5: 
		print "<lstfile> <spi or mrcs filename> <ny> <apix> <output> <metadata line> <1,odd 2,even 0,none>"
		sys.exit(-1)
	...
	output=args[4]
        meta=int(args[5])
        debug=int(args[6])
	return (lstfile, filename,ny,apix,output,meta,debug)
